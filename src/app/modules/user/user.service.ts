...
const updateUserIntoDB = async (id: string, payload: any, profileImage: any, logoImage: any, coverImage: any) => {
    try {
        const findUser = await prisma.user.findUnique({
            where: {
                id
        ...
        throw new ApiError(StatusCodes.BAD_REQUEST, "User not found, update failed")
    }
}
